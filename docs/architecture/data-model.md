...
`DataContainer` owns operations that read or mutate its tree, including insertion, removal, movement, path-based updates, change handling, and order initialisation. Top-level field ordering is stored in `metadata.order` as field identifiers. A section has no separate order array; `SectionField` rebuilds `childNodes` in the intended object-key insertion order when inserting or moving a child.
...
The abstract `field_data<T>` class owns common field storage, shallow cloning, and abstract data/serialization methods. Its base movement and deletion methods are currently empty; tree ownership lives on `DataContainer` for top-level nodes and `SectionField` for section children. Concrete classes cover text, number, time, date, duration, scale, boolean, image-boolean, section, and selection fields.
...
The earlier `NodeTypes.ts`, `TypedNode.tsx`, and separate field/section factory architecture is retired. Files under `old code/` are not an active compatibility layer. `components/nodes/operations/Component.tsx` remains as an unused duplicate; active imports use `components/common/Component.tsx`.
